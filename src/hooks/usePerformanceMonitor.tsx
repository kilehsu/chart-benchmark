...
function computeMetrics(
  ...
  if (times.length === 0) {
    return { fps: 0, p95FrameMs: 0, longFrames: 0, hz: 0 };
  }
...
export function PerformanceMonitor() {
  ...
  useEffect(() => {
    intervalId.current = setInterval(() => {
      const result = computeMetrics(frameTimes.current, frameBudget.current);
      ...
      ctx.fillStyle = "#111";
      ctx.fillText(`FPS: ${result.fps}`, 10, 20);
      ctx.fillText(`p95: ${result.p95FrameMs.toFixed(2)}ms`, 10, 40);
      ctx.fillText(`Long frames: ${result.longFrames}`, 10, 60);
      ctx.fillText(`Hz: ${result.hz}`, 10, 80);
    }, 1000);
